...
## `vp pm stage publish ./packages/a --recursive --dry-run --registry http://127.0.0.1:9 -- --fetch-retries 0`

Reject an explicit folder with recursive before npm can run prepublishOnly.

**Exit code:** 1

```
npm staged publishing cannot combine --recursive or --filter with an explicit tarball or folder other than '.'.
```

## `vpt stat-file prepublish-ran --assert missing`

```
prepublish-ran: missing
```

## `vp pm stage publish ./packages/a --filter vp-stage-workspace-b --dry-run --registry http://127.0.0.1:9 -- --fetch-retries 0`

Reject an explicit folder with a filter rather than silently staging the wrong package.
