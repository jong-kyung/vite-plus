...
mod tests {
    ...
    fn test_yarn_logout_rejects_unsupported_selectors() {
        ...
        let args = parse_args::<LogoutArgs>(["--scope", "company"]).unwrap();
        expect_unsupported(
    ...
    #[test]
    fn test_logout_preserves_supported_selectors() {
        let args = parse_args::<LogoutArgs>([
            "--registry",
            "https://registry.example.com",
            "--scope",
        ...
        for resolution in [
            ...
            assert_eq!(
                command.args,
                vec!["logout", "--registry", "https://registry.example.com", "--scope", "@company"]
            );
        }
        for version in ["2.4.2", "3.6.0", "4.18.0"] {
            let args = parse_args::<LogoutArgs>(["--scope", "company", "--", "--publish"]).unwrap();
    ...
    #[test]
    fn test_logout_preserves_raw_selectors() {
