                ...
                let pattern = if field_support.is_empty() {
                    ...
                    let bindings = field_support
                        ...
                        .map(|(index, support)| {
                            ...
                            let binding =
                                Ident::new(&format!("__vp_pm_cli_arg_{index}"), Span::mixed_site());
                            (field, binding)
                        })
                        .collect::<Vec<_>>();
...
mod tests {
    ...
    fn enum_bindings_do_not_shadow_generated_names() {
        ...
        assert!(output.contains("diag : __vp_pm_cli_arg_0"));
        assert!(output.contains("dialect : __vp_pm_cli_arg_1"));
        assert!(output.contains("rules : __vp_pm_cli_arg_2"));
    }
